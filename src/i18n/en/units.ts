export default {
  ...
  'about.bit':
    'The smallest unit of data in computing, representing a single binary value of 0 or 1. For example, a single bit can indicate a simple yes/no or on/off state.',
  'about.nibble':
    'Comprises 4 bits. A nibble can represent up to 16 different values (2^4), often used to represent a single character in hexadecimal notation. For example, the hexadecimal digit "F" in a color code for web design.',
  'about.byte':
    'Consists of 8 bits. A byte can store a single character, such as a letter, number, or symbol. For example, the letter "A" or the symbol "&".',
  'about.KB':
    'A kilobyte (1,024 bytes) can store a short paragraph of text. For example, a small text file containing a recipe or an email.',
  'about.MB':
    'A megabyte (1,024 KB) can store a medium-quality photograph or a minute of standard-quality audio. For example, an MP3 song file or a digital photo.',
  'about.GB':
    'A gigabyte (1,024 MB) can store a feature-length movie or several hundred songs. For example, a digital video file of a movie or a small video game.',
  'about.TB':
    'A terabyte (1,024 GB) can store large databases or extensive collections of high-definition videos and games. For example, the storage capacity of a personal computer or a small data center.',
  'about.PB':
    'A petabyte (1,024 TB) can store all the academic research papers ever written or comprehensive datasets for big data analysis. For example, the data storage needs of a large social media platform.',
  'about.EB':
    'An exabyte (1,024 PB) can store vast amounts of digital information, such as all the words ever spoken by humans. For example, the data handled by global internet traffic in a few days.',
  'about.ZB':
    'A zettabyte (1,024 EB) can store the sum of all digital data currently existing. For example, the amount of data generated by all internet users over several years.',
  'about.YB':
